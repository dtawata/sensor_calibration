...
const getUniqueBaseStations = async ({ start, end }) => {
  try {
    const queryString = 'SELECT DISTINCT base_station_id FROM base_station_calibration_data WHERE calibration_date BETWEEN $1 AND $2 ORDER BY base_station_id';
    const queryArgs = [start, end];
    const res = await pool.query(queryString, queryArgs);
    return res.rows;
  } catch (err) {
...
const getUniqueAlgorithmVersionsSCO = async ({ start, end, calibration }) => {
  try {
    let queryString = 'SELECT DISTINCT algorithm_version FROM sensor_calibration_output WHERE generation_date BETWEEN $1 AND $2 AND calibration_file = $3 ORDER BY algorithm_version DESC';
    let queryArgs = [start, end, calibration];
    if (calibration === '*') {
      queryString = 'SELECT DISTINCT algorithm_version FROM sensor_calibration_output WHERE generation_date BETWEEN $1 AND $2 ORDER BY algorithm_version DESC';
      queryArgs = [start, end];
    }
    const res = await pool.query(queryString, queryArgs);
    return res.rows;
...
const getUniqueAlgorithmVersionsPVO = async ({ start, end }) => {
  try {
    const queryString = 'SELECT DISTINCT algorithm_version FROM performance_validation_output WHERE validation_date BETWEEN $1 AND $2 ORDER BY algorithm_version DESC';
    const queryArgs = [start, end];
    const res = await pool.query(queryString, queryArgs);
    return res.rows;
  } catch (err) {
